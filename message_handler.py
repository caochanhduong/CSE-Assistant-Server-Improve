...
def check_match_obj(constraint_obj,value_obj):
    match = True
    for key in list(value_obj.keys()):
        if constraint_obj[key] != '' and not check_match_sublist_and_substring(constraint_obj[key],value_obj[key]):
            match = False
            break
    return match
...
def process_message_to_user_request(message,state_tracker):
    ...
    if isinstance(message,str):
        ...
        if intent not in ['hello','done','not intent','thanks','anything',"other"]:
            ...
            ######TO DO : lúc user inform 1 key thì tạo thêm list_match_obj (với chỉ 1 key và nhét vào user action) (ok)
